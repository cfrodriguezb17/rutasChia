                ...
                <div class="md:grid md:grid-cols-3 md:gap-6">
                    <div class="md:col-span-1 mb-3">
                        <div class="px-4 sm:px0">
                            <h3 class="text-lg text-gray-900">Agregar Carro</h3>
                            <p class="text-sm text-gray-600">Ingresa la información del carro que quieres añadir</p>
                            <div class="mt-3">
                        ...
                        <div class="shadow bg-white md:rounded-md p-4">
                            <form @submit.prevent="submit">
                              ...
                              <input class="hidden" type="file" id="imageStudent" accept=".jpg, .png" name="image" @change="onFileChange" @input="form.image = $event.target.files[0]">
                               ...
                               <input type="text" class="form-input w-full rounded-md shadow-sm" v-model="form.brand">
                               <label class="block font-medium text-sm text-gray-700 my-2">
                                   Modelo de la marca 
                               </label> 
                               <input type="text" class="form-input w-full rounded-md shadow-sm" v-model="form.mob">
                               <label class="block font-medium text-sm text-gray-700 my-2">
                                   Año 
                               </label> 
                               <input type="number" class="form-input w-full rounded-md shadow-sm" min="1980" max="2023" step="1" v-model="form.year">
                                <label class="block font-medium text-sm text-gray-700 my-2">
                                   Empresa del carro 
                               </label> 
                               <input type="text" class="form-input w-full rounded-md shadow-sm" v-model="form.company">
                                <div class="btn-file flex">
                                    <button class="mx-auto place-content-center cursor-pointer mt-4 bg-yellow-300 hover:bg-yellow-600 text-black hover:text-white font-bold py-2 px-4 rounded">Agregar</button>
                              </div>                                                                                           
                            </form>
                        </div>
                    </div>
        ...
        data() {
            return {
                form: {
                    ...
                    year: '',
                    company: '',
                    image: 'cars/default-car.jpg',
                },
                urlImage: '../../storage/cars/default-car.jpg',
            }
        },
        methods: {
            onFileChange(e) {
